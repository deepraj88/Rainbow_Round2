...
//    uint8_t * mat_l2_F2 = aligned_alloc( 32 , _O1*_O2_BYTE );
  ...
  //  memcpy( digest_salt , _digest , _HASH_LEN );
    ...
    while( !succ ) {
        ...
        rainbow_sign_label17:for(k = 0; k < _O1_BYTE; k++) temp_o[k] = r_l1_F1[k];
...
int rainbow_verify( const uint8_t * digest , const uint8_t * signature , const unsigned char * pk )
{
   ...
   // memcpy( digest_salt , digest , _HASH_LEN );
    ...
    int k;
    rainbow_verify_label12:for(k = 0; k <_HASH_LEN;k++)digest_salt[k] = digest[k];
    rainbow_verify_label13:for(k=0;k<_SALT_BYTE;k++)(digest_salt+_HASH_LEN)[k] = (signature+_PUB_N_BYTE)[k];
