...
void generate_keypair(ext_cpk_t * xpk,unsigned char rpk[sizeof(pk_t)], unsigned char sk[sizeof(sk_t)], const unsigned char sk_seed[32] )
{
	pk_t pk_tmp;
	sk_t sk_tmp;
	int loop;
 ...
 //   ext_cpk_t  xpk ;
//    ext_cpk_t * pk = (ext_cpk_t*) aligned_alloc( 32 , sizeof(ext_cpk_t) );
    calculate_Q_from_F(xpk, &sk_tmp , &sk_tmp );   // compute the public key in ext_cpk_t format.
    calculate_t4( sk_tmp.t4 , sk_tmp.t1 , sk_tmp.t3 );

    obsfucate_l1_polys( xpk->l1_Q1 , xpk->l2_Q1 , N_TRIANGLE_TERMS(_V1) , sk_tmp.s1 );
    obsfucate_l1_polys( xpk->l1_Q2 , xpk->l2_Q2 , _V1*_O1 , sk_tmp.s1 );
    obsfucate_l1_polys( xpk->l1_Q3 , xpk->l2_Q3 , _V1*_O2 , sk_tmp.s1 );
    obsfucate_l1_polys( xpk->l1_Q5 , xpk->l2_Q5 , N_TRIANGLE_TERMS(_O1) , sk_tmp.s1 );
    obsfucate_l1_polys( xpk->l1_Q6 , xpk->l2_Q6 , _O1*_O2 , sk_tmp.s1 );
    obsfucate_l1_polys( xpk->l1_Q9 , xpk->l2_Q9 , N_TRIANGLE_TERMS(_O2) , sk_tmp.s1 );
//    // so far, the pk ontains the full pk but in ext_cpk_t format.
//
    extcpk_to_pk( &pk_tmp , xpk );     // convert the public key from ext_cpk_t to pk_t.
//    free( pk );
    generate_keypair_label12:for(loop = 0; loop < sizeof(pk_t);loop++)
    {
    ...
    generate_keypair_label14:for(loop = 0 ; loop<_O2_BYTE * N_TRIANGLE_TERMS(_O1);loop++)
    {
    ...
    generate_keypair_label13:for(loop = 0;loop<_O2_BYTE * _O1*_O2; loop++)
