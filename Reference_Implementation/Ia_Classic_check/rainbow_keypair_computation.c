...
void extcpk_to_pk( pk_t * pk , const ext_cpk_t * cpk )
{
    ...
    for(i=0;i<_V1;i++) {
        for(j=i;j<_V1;j++) {
            ...
            for(k=0;k<_O1_BYTE;k++)(& pk->pk[ _PUB_M_BYTE*pub_idx ] )[k] = cpk->l1_Q1[idx_l12+k];
            for(k=0;k<_O2_BYTE;k++)((&pk->pk[ _PUB_M_BYTE*pub_idx ]) + _O1_BYTE)[k] = cpk->l2_Q1[idx_l22+k];
            idx_l12 += _O1_BYTE;
            idx_l22 += _O2_BYTE;
        }
    }
    ...
    for(i=0;i<_V1;i++) {
        for(j=_V1;j<_V1+_O1;j++) {
            unsigned pub_idx = idx_of_trimat(i,j,_PUB_N);
            for(k=0;k<_O1_BYTE;k++)(& pk->pk[ _PUB_M_BYTE*pub_idx ] )[k] = cpk->l1_Q2[idx_l12+k];
            for(k=0;k<_O2_BYTE;k++)((&pk->pk[ _PUB_M_BYTE*pub_idx ]) + _O1_BYTE)[k] = cpk->l2_Q2[idx_l22+k];
            idx_l12 += _O1_BYTE;
            idx_l22 += _O2_BYTE;
        }
    }
    ...
    for(i=0;i<_V1;i++) {
        for(j=_V1+_O1;j<_PUB_N;j++) {
            unsigned pub_idx = idx_of_trimat(i,j,_PUB_N);
            for(k=0;k<_O1_BYTE;k++)(& pk->pk[ _PUB_M_BYTE*pub_idx ] )[k] = cpk->l1_Q3[idx_l12+k];
            for(k=0;k<_O2_BYTE;k++)((&pk->pk[ _PUB_M_BYTE*pub_idx ]) + _O1_BYTE)[k] = cpk->l2_Q3[idx_l22+k];
            idx_l12 += _O1_BYTE;
            idx_l22 += _O2_BYTE;
        }
    }
    ...
    for(i=_V1;i<_V1+_O1;i++) {
        for(j=i;j<_V1+_O1;j++) {
            unsigned pub_idx = idx_of_trimat(i,j,_PUB_N);
            for(k=0;k<_O1_BYTE;k++)(& pk->pk[ _PUB_M_BYTE*pub_idx ] )[k] = cpk->l1_Q5[idx_l12+k];
            for(k=0;k<_O2_BYTE;k++)((&pk->pk[ _PUB_M_BYTE*pub_idx ]) + _O1_BYTE)[k] = cpk->l2_Q5[idx_l22+k];
            idx_l12 += _O1_BYTE;
            idx_l22 += _O2_BYTE;
        }
    }
    ...
    for(i=_V1;i<_V1+_O1;i++) {
        for(j=_V1+_O1;j<_PUB_N;j++) {
            unsigned pub_idx = idx_of_trimat(i,j,_PUB_N);
            for(k=0;k<_O1_BYTE;k++)(& pk->pk[ _PUB_M_BYTE*pub_idx ] )[k] = cpk->l1_Q6[idx_l12+k];
            for(k=0;k<_O2_BYTE;k++)((&pk->pk[ _PUB_M_BYTE*pub_idx ]) + _O1_BYTE)[k] = cpk->l2_Q6[idx_l22+k];
            idx_l12 += _O1_BYTE;
            idx_l22 += _O2_BYTE;
        }
    }
    ...
    for(i=_V1+_O1;i<_PUB_N;i++) {
        for(j=i;j<_PUB_N;j++) {
            unsigned pub_idx = idx_of_trimat(i,j,_PUB_N);
            for(k=0;k<_O1_BYTE;k++)(& pk->pk[ _PUB_M_BYTE*pub_idx ] )[k] = cpk->l1_Q9[idx_l12+k];
            for(k=0;k<_O2_BYTE;k++)((&pk->pk[ _PUB_M_BYTE*pub_idx ]) + _O1_BYTE)[k] = cpk->l2_Q9[idx_l22+k];
            idx_l12 += _O1_BYTE;
            idx_l22 += _O2_BYTE;
        }
    }
...
//    memset( Qs->l1_Q9 , 0 , _O1_BYTE * N_TRIANGLE_TERMS(_O2) );
    ...
    //unsigned char * tempQ = (unsigned char *) malloc(size_tempQ + 32 );
    //printf(" \n tempQ = %d \n ",size_tempQ + 32);
    unsigned char tempQ[16416];
//    unsigned char * tempQ = (unsigned char *) aligned_alloc( 32 , size_tempQ + 32 );

    //memset( tempQ , 0 , _O1_BYTE * _O1 * _O1 );   // l1_Q5
   // int k;
    for(k=0;k<_O1_BYTE * _O1 * _O1 ;k++)
    {
    	tempQ[k]=0;
    }
...
*/
    batch_trimat_madd( Qs->l1_Q3 , Fs->l1_F1 , Ts->t4 , _V1, _V1_BYTE , _O2, _O1_BYTE );         // F1*T2
    batch_mat_madd( Qs->l1_Q3 , Fs->l1_F2 , _V1, Ts->t3 , _O1, _O1_BYTE , _O2, _O1_BYTE );   // F1_T2 + F2_T3

  //  memset( tempQ , 0 , _O1_BYTE * _O2 * _O2 );                                              // l1_Q9
    for(k=0;k<_O1_BYTE * _O2 * _O2;k++)
    {
    	tempQ[k] = 0;
    }
    batch_matTr_madd( tempQ , Ts->t4 , _V1, _V1_BYTE, _O2, Qs->l1_Q3, _O2, _O1_BYTE );           // T2tr * ( F1_T2 + F2_T3 )
    UpperTrianglize( Qs->l1_Q9 , tempQ , _O2 , _O1_BYTE );                                   // Q9

    batch_trimatTr_madd( Qs->l1_Q3 , Fs->l1_F1 , Ts->t4 , _V1, _V1_BYTE, _O2, _O1_BYTE );        // F1_F1T_T2 + F2_T3  // Q3

    batch_bmatTr_madd( Qs->l1_Q6 , Fs->l1_F2, _O1, Ts->t4, _V1, _V1_BYTE, _O2, _O1_BYTE );       // F2tr*T2
...
*/
    ...
    for (k =0;k<_O2_BYTE * N_TRIANGLE_TERMS(_V1) ;k++)
    {
    	Qs->l2_Q1[k]  = Fs->l2_F1[k];
    }
    //memcpy( Qs->l2_Q2 , Fs->l2_F2 , _O2_BYTE * _V1 * _O1 );
    for (k =0;k<_O2_BYTE * _V1 * _O1 ;k++)
        {
        	Qs->l2_Q2[k]  = Fs->l2_F2[k];
        }
  ...
  //  memcpy( Qs->l2_Q5 , Fs->l2_F5 , _O2_BYTE * N_TRIANGLE_TERMS(_O1) );
    for (k =0;k<_O2_BYTE * N_TRIANGLE_TERMS(_O1)  ;k++)
            {
            	Qs->l2_Q5[k]  = Fs->l2_F5[k];
            }
  //  memset( tempQ , 0 , _O2_BYTE * _O1 * _O1 );                                               // l2_Q5
    for(k=0;k< _O2_BYTE * _O1 * _O1;k++)
    {
    	tempQ[k] =0;
    }
...
*/
   // memcpy( Qs->l2_Q3 , Fs->l2_F3 , _O2_BYTE * _V1 * _O2 );
    ...
    batch_trimat_madd( Qs->l2_Q3 , Fs->l2_F1 , Ts->t4 , _V1, _V1_BYTE , _O2, _O2_BYTE );         // F1*T2 + F3
   ...
   for(k=0;k<_O2_BYTE * _O2 * _O2 ; k++) tempQ[k]=0;
    batch_matTr_madd( tempQ , Ts->t4 , _V1, _V1_BYTE, _O2, Qs->l2_Q3, _O2, _O2_BYTE );           // T2tr * ( ..... )
   ...
   // memset( Qs->l2_Q9 , 0 , _O2_BYTE * N_TRIANGLE_TERMS(_O2) );
    ...
    batch_trimatTr_madd( Qs->l2_Q3 , Fs->l2_F1 , Ts->t4 , _V1, _V1_BYTE, _O2, _O2_BYTE );        // F1_F1T_T2 + F2_T3 + F3 // Q3

    batch_bmatTr_madd( Qs->l2_Q6 , Fs->l2_F2, _O1, Ts->t4, _V1, _V1_BYTE, _O2, _O2_BYTE );       //  F5*T3 + F6 +  F2tr*T2
